...
7. No origin allowlist change is needed while the website proxies server-side. If browser-side calls are ever wanted, the origins to add are `https://niqsng.org`, `https://www.niqsng.org`, and `https://niqs-website.vercel.app` (plus preview URLs, which are per-deployment and would need a wildcard).
